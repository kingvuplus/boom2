...
def InitUsageConfig():
    ...
    config.workaround.blueswitch = ConfigSelection(default='0', choices=[('0', _('BluePanel/Extensions')), ('1', _('Extensions/BluePanel'))])
    ...
    infobarpredefinedlist = []
    infobarsecpredefinedlist = []
    f = os.listdir('/usr/share/enigma2/' + config.skin.primary_skin.value[:-8])
    if f:
        for line in f:
            parts = line.split()
            path = '/usr/share/enigma2/' + config.skin.primary_skin.value[:-8]
            infobar = parts[0][:-4]
            secinfobar = parts[0][:-4]
            if infobar.find('skin_infobar_') >= 0:
                infobar2 = parts[0][13:-4]
                print infobar2
                infobarpredefinedlist.append(infobar2)
            if secinfobar.find('skin_2infobar_') >= 0:
                secinfobar2 = parts[0][14:-4]
                print secinfobar2
                infobarsecpredefinedlist.append(secinfobar2)

        infobarpredefinedlist.sort()
        infobarsecpredefinedlist.sort()
    config.usage.show_infobar_lite = ConfigSelection(default='InfoBar', choices=[('InfoBar', _('InfoBar'))] + infobarpredefinedlist)
    config.usage.show_second_infobar = ConfigSelection(default='SecondInfoBar', choices=[('Off', _('Off')),
     ('EventInfo', _('EventInfo')),
     ('2ndINFO', _('2ndINFO')),
     ('SecondInfoBar', _('SecondInfoBar'))] + infobarsecpredefinedlist)
    ...
    config.usage.serviceitems_per_page = ConfigSelectionNumber(default=13, stepwidth=1, min=3, max=40, wraparound=True)
    ...
    config.epg.saveepg = ConfigYesNo(default=False)
    ...
    config.italysat = ConfigSubsection()
    config.italysat.shownetdet = ConfigYesNo(default=True)
    config.italysat.fp = ConfigText(default='')
    config.italysat.skindevelopermode = ConfigYesNo(default=False)
    ...
    config.plugins.MetrixWeather.woeid = ConfigNumber(default='721943')
    ...
    config.usage.inactivity_timer_blocktime_begin = ConfigClock(default=mktime((0,
     0,
     0,
     6,
     0,
     ...
     0,
     0)))
    config.usage.inactivity_timer_blocktime_end = ConfigClock(default=mktime((0,
     0,
     0,
     23,
     0,
    ...
    config.usage.standby_to_shutdown_timer_blocktime_begin = ConfigClock(default=mktime((0,
     0,
     0,
     6,
     0,
     ...
     0,
     0)))
    config.usage.standby_to_shutdown_timer_blocktime_end = ConfigClock(default=mktime((0,
     0,
     0,
     23,
     0,
