# Embedded file name: /usr/lib/enigma2/python/ItalySat/ItalysatDevice.py
...
class ItalyDevicesPanel(Screen):
    ...
    def createSummary(self):
        return ItalyDevicesPanelSummary
    ...
    def updateList(self, result = None, retval = None, extra_args = None):
        ...
        if fileExists('/usr/lib/enigma2/python/Plugins/Extensions/ItalyBoot/.italyboot_location'):
            f = open('/usr/lib/enigma2/python/Plugins/Extensions/ItalyBoot/.italyboot_location', 'r')
            mypath = f.readline().strip()
            f.close()
            if not path.exists('/media/italyboot'):
                system('mkdir /media/italyboot && mount ' + mypath + ' /media/italyboot')
    ...
    def SetupMounts(self):
        self.session.openWithCallback(self.updateList, ItalyDevicesConf)

    def Format(self):
        from ItalysatUsbFormat import UsbFormat
        self.session.openWithCallback(self.updateList, UsbFormat)
...
class ItalyDevicesConf(Screen, ConfigListScreen):
...
class ItalyDevicesPanelSummary(Screen):
