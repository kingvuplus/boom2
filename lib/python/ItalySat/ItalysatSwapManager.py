# Embedded file name: /usr/lib/enigma2/python/ItalySat/ItalysatSwapManager.py
...
import stat
config.italysat.swapautostart = ConfigYesNo(default=False)
startswap = None

def SwapAutostart(reason, session = None, **kwargs):
    global startswap
    if reason == 0:
        if config.italysat.swapautostart.value:
...
class ItalySwapPanel(Screen):
    ...
    def getSwapDevice(self):
        ...
        if path.exists('/etc/rcS.d/S98SwapManager'):
            ...
            config.italysat.swapautostart.value = True
            config.italysat.swapautostart.save()
    ...
    def updateSwap2(self, result = None, retval = None, extra_args = None):
        ...
        if config.italysat.swapautostart.value and self.swap_place:
            self['autostart_off'].hide()
            self['autostart_on'].show()
        else:
            config.italysat.swapautostart.setValue(False)
            config.italysat.swapautostart.save()
            configfile.save()
            self['autostart_on'].hide()
            self['autostart_off'].show()
        ...
        if retval == 0:
            try:
                ...
                if config.italysat.swapautostart.value:
                    ...
                    config.italysat.swapautostart.save()
                    configfile.save()
                mybox = self.session.open(MessageBox, _('Swap successfully removed.'), MessageBox.TYPE_INFO)
                mybox.setTitle(_('Info'))
                self.updateSwap()
            except:
                mybox = self.session.open(MessageBox, _('Failed to remove Swap, maybe no r/w permission'), MessageBox.TYPE_INFO)
                mybox.setTitle(_('Info'))
    ...
    def autoSsWap(self):
        if self.swap_place:
            if config.italysat.swapautostart.value:
                config.italysat.swapautostart.value = False
                config.italysat.swapautostart.save()
            else:
                config.italysat.swapautostart.value = True
                config.italysat.swapautostart.save()
