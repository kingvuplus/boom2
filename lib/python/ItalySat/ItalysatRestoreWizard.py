# Embedded file name: /usr/lib/enigma2/python/ItalySat/ItalysatRestoreWizard.py
...
class ItalyRestoreWizard(WizardLanguage, Rc):
    ...
    def doRestorePluginsTestComplete(self, result = None, retval = None, extra_args = None):
        ...
        if result.find('No space left on device') != -1:
            self.NextStep = 'reboot'
            self.buildListRef = self.session.openWithCallback(self.buildListfinishedCB, MessageBox, _('Your %s %s have a FULL flash memory, please free memory or expand in USB') % (getMachineBrand(), getMachineName()), type=MessageBox.TYPE_INFO, timeout=30, wizard=True)
            self.buildListRef.setTitle(_('Restore Wizard'))
