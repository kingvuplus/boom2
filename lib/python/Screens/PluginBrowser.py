...
config.pluginfilter.kernel = ConfigYesNo(default=True)
...
def Check_Softcam():
    found = False
    for x in os.listdir('/usr/emuscript'):
        if x.find('.sh') > -1:
            found = True
            break
...
class PluginDownloadBrowser(Screen):
    ...
    def createPluginFilter(self):
        ...
        if config.pluginfilter.e2_locales.value:
    ...
    def dataAvail(self, str):
        if self.type == self.DOWNLOAD and str.find('404 Not Found') >= 0:
            ...
            self.run = 3
            return
        if self.type == self.DOWNLOAD and str.find('No space left on device') >= 0:
            self['text'].setText(_('You have a FULL flash memory, please free memory or expand in USB'))
            self.run = 3
            return
