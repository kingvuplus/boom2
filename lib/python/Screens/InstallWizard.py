...
class InstallWizard(Screen, ConfigListScreen):
    ...
    def run(self):
        ...
        elif self.index == self.STATE_CHOISE_SETTINGPLUGIN and self.enabled.value:
            self.session.open(InstallWizardIpkgUpdater, self.index, _('Please wait (downloading plugin)'), IpkgComponent.CMD_INSTALL, {'package': 'enigma2-plugin-extensions-' + self.pluginsetting_type.value})
        elif self.index == self.STATE_CHOISE_MULTIBOOTPLUGIN and self.enabled.value:
            self.session.open(InstallWizardIpkgUpdater, self.index, _('Please wait (downloading plugin)'), IpkgComponent.CMD_INSTALL, {'package': 'enigma2-plugin-extensions-' + self.pluginmultiboot_type.value})
...
class InstallWizardIpkgUpdater(Screen):
    ...
    def __init__(self, session, index, info, cmd, pkg = None):
        ...
        if self.index == InstallWizard.STATE_CHOISE_CHANNELLIST:
            ...
            self.ipkg.startCmd(cmd, {'package': 'enigma2-plugin-extensions-*'})
        else:
