...
class InfoBar(InfoBarBase, InfoBarShowHide, InfoBarNumberZap, InfoBarChannelSelection, InfoBarMenu, InfoBarEPG, InfoBarRdsDecoder, InfoBarInstantRecord, InfoBarAudioSelection, InfoBarRedButton, InfoBarTimerButton, InfoBarINFOpanel, InfoBarResolutionSelection, InfoBarAspectSelection, InfoBarVmodeButton, HelpableScreen, InfoBarAdditionalInfo, InfoBarNotifications, InfoBarDish, InfoBarUnhandledKey, InfoBarLongKeyDetection, InfoBarSubserviceSelection, InfoBarTimeshift, InfoBarSeek, InfoBarCueSheetSupport, InfoBarBuffer, InfoBarSummarySupport, InfoBarTimeshiftState, InfoBarTeletextPlugin, InfoBarExtensions, InfoBarPiP, InfoBarPlugins, InfoBarSubtitleSupport, InfoBarServiceErrorPopupSupport, InfoBarJobman, InfoBarZoom, InfoBarHdmi, InfoBarButtonSetup, Screen):
    ...
    def __init__(self, session):
        ...
        self['actions'] = HelpableActionMap(self, 'InfobarActions', {'showMovies': (self.showMovies, _('Play recorded movies...')),
         ...
         'showBoxPortal': (self.showBoxPortal, _('Show Box Portal...'))}, prio=2)
        self['key_red'] = Label()
    ...
    def doButtonsCheck(self):
        if config.plisettings.ColouredButtons.value:
            if config.plugins.infopanel_redpanel.selection.value == '0':
                ...
                self['key_red'].setText(_('Movies'))
            if config.plugins.infopanel_yellowkey.list.value == '0':
                self['key_yellow'].setText(_('Audio'))
            elif config.plugins.infopanel_yellowkey.list.value == '1':
                self['key_yellow'].setText(_('Timeshift'))
            elif config.plugins.infopanel_yellowkey.list.value == '2':
                self['key_yellow'].setText(_('Videomode'))
    ...
    def showTvButton(self):
        if boxtype.startswith('gb') or boxtype in ('classm', 'genius', 'evo', 'galaxym6'):
            self.toogleTvRadio()
        elif boxtype in ('uniboxhd1', 'uniboxhd2', 'uniboxhd3', 'sezam5000hd', 'mbtwin', 'cloudibox2plus', 'cloudibox3'):
            self.showMovies()
        else:
            self.showTv()
...
class MoviePlayer(InfoBarAspectSelection, InfoBarSimpleEventView, InfoBarBase, InfoBarShowHide, InfoBarLongKeyDetection, InfoBarMenu, InfoBarEPG, InfoBarSeek, InfoBarShowMovies, InfoBarInstantRecord, InfoBarAudioSelection, HelpableScreen, InfoBarNotifications, InfoBarServiceNotifications, InfoBarPVRState, InfoBarCueSheetSupport, InfoBarMoviePlayerSummarySupport, InfoBarSubtitleSupport, Screen, InfoBarTeletextPlugin, InfoBarServiceErrorPopupSupport, InfoBarExtensions, InfoBarPlugins, InfoBarPiP, InfoBarZoom, InfoBarHdmi, InfoBarButtonSetup):
